...
?>
 ...
 <div class="col-12 col-md-9 shadow-lg p-3 mb-5 bg-white rounded" >
        ...
        <div class="row"><a class="btn btn-secondary mr-2" href="mojzbior.php" role="button">Mój zbiór</a>
        <div ><a class="btn btn-secondary ml-2 mr-5" href="toLend.php" role="button">Wystawione</a></div>
            <label  class="h5  mt-1">Wystawione:</label> </div>
    ...
    inner join Kategorie ka on ka.id_kategoria=k.id_kategoria  Where w.id_czytelnik_dawca=$id and w.status=1; ");
    ...
    if($_SESSION['typ']==2)
    {
        $id=$_SESSION['ID']; 
    $q = mysqli_query($link, "select * from Transakcje w  inner join Ksiazki k on k.id_ksiazka=w.id_ksiazka inner join Autorzy a on a.id_autor=k.id_autor  
    inner join Kategorie ka on ka.id_kategoria=k.id_kategoria  Where w.id_biblioteka_dawca=$id and w.status=1;");
